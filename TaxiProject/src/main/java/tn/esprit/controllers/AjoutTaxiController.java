...
public class AjoutTaxiController {
    ...
    void ajouterTaxi(ActionEvent event) {
        ...
        try {
            ...
            serviceTaxi.ajouter(nouveauTaxi);
    ...
    private boolean validerFormulaire() {
        ...
        try {
            if (serviceTaxi.immatriculationExiste(txtImmatriculation.getText(), null)) {
                afficherAlerte("Duplicata", "Cette immatriculation existe déjà !", Alert.AlertType.WARNING);
                return false;
            }

            if (serviceTaxi.licenceExiste(txtLicenceNumero.getText(), null)) {
                afficherAlerte("Duplicata", "Ce numéro de licence existe déjà !", Alert.AlertType.WARNING);
                return false;
            }
        }
        catch (SQLException e) {
            afficherAlerte("Erreur BD", "Impossible de vérifier les données", Alert.AlertType.ERROR);
            return false;
        }
